let metaddr = {  
  metmarket: "0xe99857b89F6305Cf03F050EC75149A17FF2A9dAe" //tiger
...
let metabi = {
  ...
  metmarket: [
   ...
   "function myinfo(address _user) public view returns (uint256,address) ",
...
let Memberjoin = async () => {
  ...
  try {
    await meta5Contract.memberjoin(document.getElementById('mentoaddress').value);
  } catch(e) {
    alert(e.data.message.replace('execution reverted: ',''))
  }
};
...
let MemberLogin = async () => {
  ...
  let mydepo =  (await my[0]);
  let mymento =  (await my[1]);
  document.getElementById("Mydepo").innerHTML = (mydepo/2e18);
  document.getElementById("Mymento").innerHTML = (mymento);
};

let Mentowithdraw = async () => {
  let userProvider = new ethers.providers.Web3Provider(window.ethereum, "any");
  await window.ethereum.request({
    method: "wallet_addEthereumChain",
    params: [{
  ...
  try {
    await meta5Contract.mentowithdraw();
  } catch(e) {
    alert(e.data.message.replace('execution reverted: ',''))
  }
};
...
async function getMenty() {
  ...
  let meta5Contract = new ethers.Contract(metaddr.metmarket, metabi.metmarket, signer);
      ...
      const mentyAddresses = await meta5Contract.getMenty(await signer.getAddress());
      displayMentyAddresses(mentyAddresses);
 
}

function displayMentyAddresses(addresses) {
  const mentyList = document.getElementById('mentyList');
  mentyList.innerHTML = '';

  addresses.forEach(address => {
      const listItem = document.createElement('li');
      listItem.textContent = address;
      mentyList.appendChild(listItem);
  });
}